...
static int test_dim_smooth[] = {32, 64, 128, 256, 512, 1024, 2048};

/* Baseline CPEs (see config.h) */
static double rotate_baseline_cpes[] = {R64, R128, R256, R512, R1024, R2048, R4096};
static double smooth_baseline_cpes[] = {S32, S64, S128, S256, S512, S1024, S2048};
...
void usage(char *progname) {
  ...
  fprintf(stderr,
          "  -g         Autograder mode: checks only rotate() and smooth()\n");
  fprintf(stderr,
          "  -f <file>  Get test function names from dump file <file>\n");
  fprintf(stderr,
          "  -d <file>  Emit a dump file <file> for later use with -f\n");
  exit(EXIT_FAILURE);
}
